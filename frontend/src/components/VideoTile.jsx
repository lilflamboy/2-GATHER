...
function VideoTile({stream,name,muted=false}){
  ...
  return(
    <div className="relative flex h-full w-full items-center justify-center overflow-hidden rounded-[1.35rem] border border-pink-200 bg-white shadow-[0_24px_60px_rgba(0,0,0,0.35)]">
      ...
      {stream
        ...
        :<div className="flex flex-col items-center gap-2 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-full border border-pink-400/18 bg-gradient-to-br from-purple-400/18 to-purple-500/12">
            <span className="font-bold text-purple-600">{name?.[0]?.toUpperCase()}</span>
          </div>
          <span className="text-xs text-zinc-600">{name}</span>
        </div>
      }
      <span className="absolute bottom-2 left-2 rounded-full border border-pink-200 bg-black/55 px-2.5 py-1 text-[11px] text-white/85 backdrop-blur-sm">{name}{muted?" (you)":""}</span>
    </div>
  );
}
